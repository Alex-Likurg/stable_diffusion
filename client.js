...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const form = document.getElementById('form'); // Находим элемент формы по ID и приводим его к типу HTMLFormElement
...
function getData() {
    return __awaiter(this, void 0, void 0, function* () {
        const inputElement = document.getElementById("input"); // Находим элемент ввода по ID и приводим его к типу HTMLInputElement
        const messagesElement = document.getElementById("messages"); // Находим элемент для отображения сообщений по ID и приводим его к типу HTMLDivElement
        ...
        if (userData === "")
            return; // Если значение пустое, прекращаем выполнение функции
        ...
        try {
            ...
            if (response.status === 200) { // Проверяем, что статус ответа 200 (OK)
                const buffer = yield response.arrayBuffer(); // Получаем тело ответа в виде массива байтов
                ...
                yield saveImageToDatabase(userData, imageStr.replace(base64Flag, '')); // Сохраняем изображение в базу данных, убирая префикс Base64
            }
                ...
                const errorText = yield response.text(); // Получаем текст ошибки из ответа
...
function saveImageToDatabase(prompt, imageBase64) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const response = yield fetch('http://localhost:3000/save-image', {
                method: 'POST', // Устанавливаем метод запроса POST
                headers: {
            ...
            const result = yield response.text(); // Получаем текстовый ответ
...
function fetchImages() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const response = yield fetch('http://localhost:3000/get-images', {
                method: 'GET',
                headers: {
...
function displayUserMessage(message) {
    const messagesContainer = document.getElementById("messages"); // Находим элемент для отображения сообщений по ID и приводим его к типу HTMLDivElement
